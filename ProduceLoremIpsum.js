...
2018-04-19
A basic attempt at producing Lorem Ipsum text.
The functionality will return a string that is stripped from a seed library at a desired length.
...
module.exports = function (context, req) {
	...
	// Desired length and unit of result
	var length=req.query.length;
	var unit=req.query.unit.toLowerCase();
	
    if (length && unit) {
		// Values to prepend and append for consistency
		...
		// If we are working with characters or words
		if(unit=='characters'){
			...
			// Split library into words [that include punctuation]
			var words=library.split(' ');
			
			// Revised length to compensate for the prepending and appending of characters [ignore append because it is just punctuation]
			length=length-prepend.split(' ').length+1;
			
			// The start position referencing the source; always be positive
			start=Math.abs(Math.floor(Math.random()*words.length)-length);
			
			// If length will exceed the length of the library, then extend the library through duplication
			if(start+length>words.length){
				var duplicate=(start+length)/words.length;
				for(var i=0;i<duplicate.toFixed(1);i++){
					words=words.concat(words);
				}
			}
			
			// Get only the relevant words from library
			var wordsToUse=[];
			for(var i=0;i<length;i++){
				wordsToUse[i]=words[start+i];
			}
			
			// The result
			generatedString=prepend+wordsToUse.join(' ').replace(/\s+$|,$|\?$|\!$|\.$/g, '')+append;
		}
        ...
        context.res = {
            status: 400,
            body: "Please pass the length and unit on the query string"
        };
    }
    context.done();
};